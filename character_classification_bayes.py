...
def main():
    ...
    train_df = pd.read_csv('data/emnist-balanced-train.csv')
    train_df = train_df[:5000]
    
    print("Opening test data frame...")
    test_df = pd.read_csv('data/emnist-balanced-test.csv')
    # Generating prediction list takes a long time, so we'll only run a small number of test cases
    test_df = test_df[:100]
    
    print("Calcualting total number of instances for each decision class...")
    class_examples = get_number_of_instances(train_df)
    total_class_examples = total_number_of_instances(class_examples)
    class_probabilities = find_decision_class_probabilities(class_examples, total_class_examples)
    
    # Declare and define vocabularies
    class_vocab = {}
    for i in range(47):
        class_vocab[i] = Counter()
    
    words_per_class = Counter()
    vocab = Counter()
    
    print("Building class vocabularies...")
    build_vocabulary(train_df, vocab, class_vocab, words_per_class)
    ...
    for decision_class in class_vocab.keys():
        prediction_list.extend(test_class_probability(test_df, class_vocab, words_per_class, total_words, class_probabilities, alpha))
